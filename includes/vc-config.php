<?php
if (!defined('ABSPATH')) exit;

add_action('vc_before_init', 'diag_resp_view_wpbakery_element');
function diag_resp_view_wpbakery_element() {
    vc_map(array(
        'name' => 'Diagonal Responsive View',
        'base' => 'diag_resp_view',
        'description' => 'Responsive template with dynamic title/content/button/media',
        'category' => 'Custom Elements',
        'icon' => 'vc_icon-wpbakery-logo',
        'params' => array(
            array('type' => 'textarea_html',
                'holder' => 'div',
                'heading' => 'Content',
                'param_name' => 'content',
                'value' => '<h2>Diagonal Responsive View</h2><p>I am test text block. Click edit button to change this text.</p>',
                'description' => 'Enter your content.',
            ),
            array('type' => 'checkbox', 
                'heading' => 'Show Button?', 
                'param_name' => 'show_button', 
                'value' => array('Yes' => 'yes'),
            ),
            array('type' => 'textfield', 
                'heading' => 'Button Text', 
                'param_name' => 'button_text', 
                'value' => 'Click Here', 
                'dependency' => array('element' => 'show_button', 'value' => 'yes'),
                'group' => 'Button Settings',
            ),
            array('type' => 'vc_link', 
                'heading' => 'Button Link', 
                'param_name' => 'button_link', 
                'dependency' => array('element' => 'show_button', 'value' => 'yes'),
                'group' => 'Button Settings',
            ),
            array('type' => 'dropdown', 
                'heading' => 'Button Alignment', 
                'param_name' => 'button_align', 
                'value' => array('Left' => 'left', 'Center' => 'center', 'Right' => 'right'),
                'dependency' => array('element' => 'show_button', 'value' => 'yes'),
                'group' => 'Button Settings',
            ),
            array('type' => 'colorpicker', 
                'heading' => 'Background Color', 
                'param_name' => 'button_bg_color', 
                'value' => '#0041C2',
                'dependency' => array('element' => 'show_button', 'value' => 'yes'),
                'group' => 'Button Settings',
            ),
            array('type' => 'colorpicker', 
                'heading' => 'Text Color', 
                'param_name' => 'button_text_color', 
                'value' => '#FFFFFF',
                'dependency' => array('element' => 'show_button', 'value' => 'yes'),
                'group' => 'Button Settings',
            ),
            array('type' => 'textfield', 
                'heading' => 'Border Radius', 
                'param_name' => 'button_border_radius', 
                'value' => '5px',
                'dependency' => array('element' => 'show_button', 'value' => 'yes'),
                'group' => 'Button Settings',
            ),
            array('type' => 'textfield', 
                'heading' => 'CSS classes', 
                'param_name' => 'button_css_classes', 
                'dependency' => array('element' => 'show_button', 'value' => 'yes'),
                'group' => 'Button Settings',
                'description' => 'Add custom CSS classes to the button, separated by spaces.',
            ),
            array('type' => 'checkbox', 
                'heading' => 'Flip media orientation?', 
                'param_name' => 'flip_media', 
                'value' => array('Yes' => 'yes'),
                'description' => 'Swap content and media order on desktop.',
            ),
            array('type' => 'dropdown', 
                'heading' => 'Mask Tilt', 
                'param_name' => 'mask_tilt', 
                'value' => array('20%' => '20', '30%' => '30', '40%' => '40'),
            ),
            array('type' => 'checkbox', 
                'heading' => 'Is media a video?', 
                'param_name' => 'is_video', 
                'value' => array('Yes' => 'yes'),
            ),
            array('type' => 'textfield', 
                'heading' => 'Video File url', 
                'param_name' => 'media_url', 
                'dependency' => array('element' => 'is_video', 'value' => array('yes')),
                'description' => 'Write the URL of the uploaded video file.',
            ),
            array('type' => 'attach_image', 
                'heading' => 'Image', 
                'param_name' => 'image_id', 
                'dependency' => array('element' => 'is_video', 'value_not_equal_to' => 'yes'),
            ),
        )
    ));
}
